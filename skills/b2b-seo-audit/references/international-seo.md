...
**EBS live status (checked 2026-08-25):** single locale — `/en/` only, `/ro/` and `/ru/` return
404, no hreflang tags, none needed. Correct as-is. This file's checklist (self-reference,
reciprocity, x-default, the Next.js `alternates.languages` caveat) becomes MANDATORY the day a
second locale ships; the locale-prefix architecture is already in place.
