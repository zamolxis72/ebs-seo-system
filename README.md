...
- Production uses the three-skill suite this repo owns — `seo-strategy` (demand), `ebs-discoverability` (execution, four instrument layers bundled), `b2b-seo-audit` (diagnosis) — plus `claim-verification` before ship; live metrics come from the Ahrefs tools, never invented.
